...
def remove_dependency(dependency_path, manifest_path):
    try:
        ...
        lm = ElementTree.Element("manifest")
    for child in lm.getchildren():
        if child.attrib['path'].endswith(dependency_path):
            print('Removing dependency %s from local manifest to avoid conflicts' % child.attrib['name'])
            lm.remove(child)
