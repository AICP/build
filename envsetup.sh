...
function eat()
{
    ...
    if (adb shell cat /system/build.prop | grep -q "ro.aicp.device=$AICP_BUILD");
    then
...
function dopush()
{
    ...
    if (adb shell cat /system/build.prop | grep -q "ro.aicp.device=$AICP_BUILD") || [ "$FORCE_PUSH" == "true" ];
    then
