...
@echo -e ${CL_MAG}"target ObjC:"${CL_RST}" $(PRIVATE_MODULE) <= $<"
$(call transform-c-or-s-to-o-no-deps)
endef
...
@echo -e ${CL_MAG}"target StaticLib:"${CL_RST}" $(PRIVATE_MODULE) ($@)"
...
@mkdir -p $(dir $@)
@echo -e ${CL_MAG}"target SharedLib:"${CL_RST}" $(PRIVATE_MODULE) ($@)"
...
@mkdir -p $(dir $@)
@echo -e ${CL_MAG}"target Strip:"${CL_RST}" $(PRIVATE_MODULE) ($@)"
...
@mkdir -p $(dir $@)
@echo -e ${CL_MAG}"target Executable:"${CL_RST}" $(PRIVATE_MODULE) ($@)"
...
@mkdir -p $(dir $@)
@echo -e ${CL_MAG}"target StaticExecutable:"${CL_RST}" $(PRIVATE_MODULE) ($@)"
...
@echo -e ${CL_MAG}"target Java:"${CL_RST}" $(PRIVATE_MODULE) ($(PRIVATE_CLASS_INTERMEDIATES_DIR))"
...
$(hide) tr ' ' '\n' < $(PRIVATE_CLASS_INTERMEDIATES_DIR)/java-source-list \
    | sort -u > $(PRIVATE_CLASS_INTERMEDIATES_DIR)/java-source-list-uniq
@echo -e ${CL_GRN}"(Incremental) build source files:"${CL_RST}""
...
@echo -e ${CL_MAG}"target Dex:"${CL_RST}" $(PRIVATE_MODULE)"
@mkdir -p $(dir $@)
$(hide) $(DX) \
...
@echo -e ${CL_MAG}"target Generated:"${CL_RST}" $(PRIVATE_MODULE) <= $<"
@mkdir -p $(dir $@)
